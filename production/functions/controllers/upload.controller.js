  ...
  uploadDoc
    ...
    .then(async (doc) => {
      if (!doc.exists) {
        ...
        if (req.user.userType === 'Personal' && doc.data().addToCV || doc.data().addToCV === 'true')
          await db
  ...
  uploadDocument
    ...
    .then(async (doc) => {
      if (doc.exists) {
        ...
        if (
          (doc.data().addToCV) || doc.data().addToCV === 'true'
        )
          await db
            .doc(
  ...
  db.doc(`/uploads/${req.params.uploadId}`)
    ...
    .then(async (doc) => {
      ...
      if (doc.data().addToCV || doc.data().addToCV === 'true')
        await db
          .doc(`/users/${doc.data().userId}/digital-cv/${req.params.uploadId}`)
          .get()
          .then(async (dc) => {
            if (dc.exists) {
              await db
  ...
  if (req.body.customUrl || req.body.customUrl === '') {
    ...
    db.collection('uploads')
      ...
      .then(async () => {
        ...
        if (req.user.userType === 'Personal' && req.body.addToCV || req.body.addToCV === 'true') {
          await db
    ...
    busboy.on('finish', async () => {
      ...
      try {
        ...
        if (newUpload.addToCV === 'true') {
          addToCV = true;
        } else if (newUpload.addToCV === 'false') addToCV = false;
        ...
        db.collection('uploads')
          ...
          .then(async () => {
            await db.doc(`mobile timeline/${docId}`).set(resUpload);
            if (userType === 'Personal' && newUpload.addToCV || newUpload.addToCV === 'true') {
              await db
                .doc(`users/${req.user.userId}/digital-cv/${docId}`)
...
exports.updateUploadDetails = (req, res) => {
  ...
  uploadDocument
    ...
    .then(async (doc) => {
      ...
      if (
        (req.user.userType === 'Personal' && doc.data().addToCV) ||
        doc.data().addToCV === 'true' ||
        true
      )
        await db
          .doc(`/users/${req.user.userId}/digital-cv/${req.params.uploadId}`)
      ...
      await db
        .doc(`/mobile timeline/${req.params.uploadId}`)
        .update(uploadDetails);
    })
    .then(() => {
